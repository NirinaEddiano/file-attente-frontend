...
const Profil = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-light">
      <ClientNavbar />
      <main className="flex max-w-7xl mx-auto p-4 profile-container">
        {/* Left Sidebar (Desktop) */}
        <aside className="w-56 pr-2 md:block hidden profile-sidebar">
          <div className="card gold-border p-4 sticky top-4">
            <nav className="space-y-2 sidebar-nav">
              {[
                { id: 'profile', label: 'Profil', icon: 'fas fa-user' },
                { id: 'password', label: 'Changer le mot de passe', icon: 'fas fa-lock' },
        ...
        <div className="md:hidden">
          <button
            ...
            className="fixed top-24 left-4 z-[1002] flex items-center p-2 bg-primary-blue text-white rounded-lg focus:ring-2 focus:ring-accent-turquoise"
            aria-label={isMenuOpen ? 'Fermer le menu de profil' : 'Ouvrir le menu de profil'}
          >
            <i className="fas fa-bars mr-2"></i> Menu
          </button>
          <div className={`profile-mobile-menu ${isMenuOpen ? 'open' : ''}`}>
            <div className="card gold-border p-4">
              <button
                onClick={() => setIsMenuOpen(false)}
                className="mb-4 text-primary-blue flex items-center"
              >
          ...
          <div
            className={`profile-mobile-menu-overlay ${isMenuOpen ? 'open' : ''}`}
            onClick={() => setIsMenuOpen(false)}
          ></div>
        </div>

        {/* Main Content */}
        <div className="w-full md:w-3/4 profile-content pt-20 md:pt-0" style={{ zIndex: 1000 }}>
          {/* Profile Header */}
          <div className="card gold-border mb-8 p-6 animate-slide-in">
            <div className="flex flex-col items-center">
              <div className="relative profile-image-container">
                ...
                />
                {editMode && (
                  <label className="absolute bottom-0 right-0 bg-accent-turquoise text-white p-2 rounded-full cursor-pointer profile-image-upload">
              ...
              <h1 className="text-2xl font-bold text-primary-blue mt-4">{userData.username}</h1>
              <p className="text-gray-600">{userData.email || 'Non défini'}</p>
            </div>
          </div>
